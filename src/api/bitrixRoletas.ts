...
async function fetchStagesFromEntity(
  ...
  }>(webhookUrl, 'crm.status.entity.items', { entityId })
...
export async function fetchStuppRoletasCatalog(
  ...
  while (true) {
    const data = await bitrixPost<{
      ...
      filter: { categoryId: Number(ROLETA_ENTITY_CATEGORY_ID) },
      select: ['id', 'title', 'stageId', 'createdTime', 'assignedById', 'categoryId'],
      order: { title: 'ASC' },
      start,
    })
